"""AppTest harness script: renders the Dashboard page in isolation.
...
# Cached function results must not leak between AppTest runs (fresh seeded
# databases per test share this same process-global cache).
st.cache_data.clear()
st.cache_resource.clear()

config = AppConfig()
dashboard.render(config=config, health_service=HealthService(config=config))
